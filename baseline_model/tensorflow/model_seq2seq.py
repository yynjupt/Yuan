...
class Seq2seq(object):
    ...
    def __init__(self, config, useTeacherForcing=True, useAttention=True):
        ...
        with tf.variable_scope("decoder"):
            ...
            tokens_eos = tf.constant(-1, tf.float32, shape=[config.batch_size, config.num_features], name='token_End') # 起始编码为-1
            tokens_go = tf.constant(-1, tf.float32, shape=[config.batch_size, config.num_features], name='token_Start') # 终止编码为-1
                ...
                if previous_state is None:    # time step == 0
                    ...
                    def get_next_input():   # time step == 1,2,3,...
                        if useTeacherForcing:  # for model training
                            prediction = self.decoder_target[:,time,:]  # [batch_size, 1, n_features]
