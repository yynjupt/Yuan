...
class Config(object):
    ...
    dropout_keep_prob = 0.7  # dropout rate
    mask_zeros = True
...
def get_batch(x_train, y_train, batch_size):
    ...
    target_batch = y_train[permutation[:batch_size],:,:]
...
if __name__ == "__main__":
    ...
    # input_name = r"D:\黑龙江火灾\Kong\b1.csv"
    ...
    x_train, y_train = load_train_data(sequence, input_length, predict_length, percent)

    print("(2) build model......")
    config = Config()
    model = Seq2seq(config=config, useTeacherForcing=True, useAttention=False)
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for batch in range(batches):
            source_batch, source_lens, target_batch, target_lens = get_batch(x_train, y_train, config.batch_size)
