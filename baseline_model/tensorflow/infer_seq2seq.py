...
def get_batch(x_test, y_test, batch_size):
    ...
    target_batch = y_test[-batch_size:, :, :]
...
if __name__ == "__main__":
    ...
    input_name = r"D:\黑龙江火灾\TestData\test_DOY_1.csv"
    out_name = r'D:\黑龙江火灾\out_Seq2Seq_without_attention.csv'
    ...
    x_test, y_test = load_test_data(sequence, input_length, predict_length, percent)


    print("(2) build model......")
    config = Config()
    model = Seq2seq(config=config, useTeacherForcing=False, useAttention=False)
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        source_batch, source_lens, target_batch, target_lens = get_batch(x_test, y_test, config.batch_size)
        ...
        #     ax.plot(target_batch[0, :, i],color='blue',marker='*',ls=':',label='real time series')
        #     ax.plot(predict_batch[0, :, i],color='red',marker='o',ls=':',label='predicted time series')
        #     ax.legend()
        for i in range(4):
            ax = plt.subplot(221+i)
            ax.scatter(target_batch[i, :, 6], target_batch[i, :, 4], color='blue', marker='*',
                       label='real time series')  # NDVI
            ax.scatter(target_batch[i, :, 6], predict_batch[i, :, 4], color='red', marker='o',
